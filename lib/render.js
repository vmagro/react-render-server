...
const {isDev, routesPath, graphql, bundleUrl} = config;
...
export default (req, res) => {
  ...
  if (isDev) {
    const query = url.parse(req, true);
    if (query.nossr != null) {
      winston.log('[dev] skipping ssr');
      res.write(renderPage('', {}, {}));
      res.end();
      return;
    }
    winston.log('[dev] hot-reloading routes');
...
function renderApp(app, context, initialState, res) {
  ...
  if (context.url) {
    ...
    res.write(renderPage(appHtml, helmet, initialState));
    res.end();
  }
}
